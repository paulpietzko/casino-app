...
export class CardEffectsService {
  ...
  applyEffect(cardElement: HTMLElement) {
    ...
    const mouseMoveListener = this.renderer.listen('document', 'mousemove', rotateToMouse);
  
    const mouseEnterListener = this.renderer.listen(cardElement, 'mouseenter', () => {
      bounds = cardElement.getBoundingClientRect();
    });
  
    const mouseLeaveListener = this.renderer.listen(cardElement, 'mouseleave', () => {
